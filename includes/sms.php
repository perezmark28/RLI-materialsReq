...
// Optional: default admin recipient (E.164 format), used for system alerts/notifications.
// Example: define('HTTPSMS_ADMIN_TO', '+63XXXXXXXXXX');
if (!defined('HTTPSMS_ADMIN_TO')) {
    // Set to the PJJ admin number you provided.
    define('HTTPSMS_ADMIN_TO', '+639178187240');
}
 ...
 */
if (!function_exists('try_shorten_url')) {
 ...
 */
if (!function_exists('httpsms_send')) {
 ...
 * Returns: [success(bool), error(string|null), raw(string|null)]
 */
if (!function_exists('httpsms_send_admin')) {
    function httpsms_send_admin(string $message): array {
        $to = trim((string)HTTPSMS_ADMIN_TO);
        if ($to === '') {
            return ['success' => false, 'error' => 'Admin recipient is not configured (HTTPSMS_ADMIN_TO).', 'raw' => null];
        }
        return httpsms_send($to, $message);
    }
}
